...
class MysqlPool(object):
    ...
    def __init__(self, db_conf=None):
        ...
        self._conn = MysqlPool.__getConn(db_conf)
        self._cursor = self._conn.cursor()
        self._cursor.execute("set autocommit=1")

    @staticmethod
    def __getConn(db_conf=None):
        """
        ...
        if MysqlPool.__pool is None:
            __pool = PooledDB(creator=MySQLdb, mincached=1, maxcached=10,
                              host=config.get(db_conf, 'host'), port=config.getint(db_conf, 'port'),
                              user=config.get(db_conf, 'user'), passwd=config.get(db_conf, 'passwd'),
                              db=config.get(db_conf, 'db'), use_unicode=True, charset=config.get(db_conf, 'charset'),
