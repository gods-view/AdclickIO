...
from affiliate.req.cake import CakeReq
...
class CakeWork(BaseWorker):
    def __init__(self, taskId, userId, url, username, password):
        BaseWorker.__init__(self, taskId, userId, url, username, password)

    def start(self):
        print (self.username, self.password)
        Cake_Req = CakeReq(url=self.url, username=self.username, password=self.password)
        flag, msg, result = Cake_Req.get_all_offer()
...
if __name__ == '__main__':
    cake = CakeWork(358, 14,
                    "https://cakemarketing.com/affiliates/api/2/offers.asmx/GetCampaign",
                    "chuck", "test")
    cake.start()
